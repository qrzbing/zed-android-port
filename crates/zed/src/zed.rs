...
use paths::{local_debug_file_relative_path, local_tasks_file_relative_path};
...
use settings::{
    ...
    initial_local_debug_tasks_content, initial_tasks_content, update_settings_file,
};
use sidebar::Sidebar;
        ...
        cx.subscribe_in(&workspace_handle, window, {
            move |workspace, _, event, window, cx| match event {
                ...
                workspace::Event::OpenBundledFile {
                    ...
                    title,
                    language,
                } => editor::open_bundled_file(workspace, text.clone(), title, language, window, cx),
                _ => {}
            }
        })
        .detach();
...
fn register_actions(
    ...
    workspace
        ...
        .register_action(editor::open_project_settings_file)
...
mod tests {
    ...
    async fn test_bundled_files_editor(cx: &mut TestAppContext) {
        ...
        cx.update(|cx| {
            cx.dispatch_action(&zed_actions::OpenDefaultSettings);
        });
        cx.run_until_parked();
            ...
            .update(cx, |_, window, cx| {
                workspace.update(cx, |workspace, cx| {
                    ...
                    editor::open_project_settings_file(
                        workspace,
                        &zed_actions::OpenProjectSettingsFile,
                        window,
                        cx,
                    );
                });
