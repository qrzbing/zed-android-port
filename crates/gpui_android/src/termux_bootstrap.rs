...
fn install_claude_setup_script(prefix: &Path) -> Result<()> {
    ...
    let body = format!(
        "#!{prefix_str}/bin/bash\n\
         ...
         # a runnable `claude` command on Android. Handles the layer-4/5\n\
         # work npm can't do automatically (musl variant, install.cjs\n\
         # platform map, patchelf interpreter, claude wrapper).\n\
         set -eu\n\
         PREFIX={prefix_str}\n\
         ...
         \n\
         if [ ! -d \"$PKG_DIR\" ]; then\n    \
             echo \"error: @anthropic-ai/claude-code not installed.\" >&2\n    \
         ...
         \n\
         echo \"==> 1. forcing install of @anthropic-ai/claude-code-linux-arm64-musl\"\n\
         npm install -g --force @anthropic-ai/claude-code-linux-arm64-musl\n\
         \n\
         echo \"==> 2. patching install.cjs to map android -> linux\"\n\
         INSTALL_CJS=\"$PKG_DIR/install.cjs\"\n\
         if grep -q \"process.platform === 'android' ? 'linux'\" \"$INSTALL_CJS\" 2>/dev/null; then\n    \
             echo \"   already patched\"\n\
         else\n    \
             [ -f \"$INSTALL_CJS.bak\" ] || cp \"$INSTALL_CJS\" \"$INSTALL_CJS.bak\"\n    \
             sed -i \"s|const platform = process\\\\.platform\\$|const platform = process.platform === 'android' ? 'linux' : process.platform|\" \"$INSTALL_CJS\"\n\
         fi\n\
         \n\
         echo \"==> 3. running install.cjs to place the musl binary\"\n\
         node \"$INSTALL_CJS\"\n\
         \n\
         echo \"==> 4. patchelf interpreter + rpath on claude.exe\"\n\
         patchelf --set-interpreter \"$MUSL_LD\" \"$CLAUDE_BIN\"\n\
         patchelf --set-rpath \"$PREFIX/lib\" \"$CLAUDE_BIN\"\n\
         \n\
         echo \"==> 5. installing $PREFIX/bin/claude wrapper\"\n\
         rm -f \"$PREFIX/bin/claude\"\n\
         printf '%s\\n' \\\n    \
             \"#!$PREFIX/bin/sh\" \\\n    \
             \"# Auto-generated by zed-setup-claude.\" \\\n    \
             \"# - env -u LD_PRELOAD: libtermux-exec.so is bionic-linked,\" \\\n    \
             \"#   loading it into musl claude.exe fails with symbol-not-found\" \\\n    \
             \"#   errors and exits early.\" \\\n    \
             \"# - proot bind: claude is a Bun-compiled musl-static binary\" \\\n    \
             \"#   whose DNS resolver hardcodes /etc/resolv.conf, which doesn't\" \\\n    \
             \"#   exist on Android. We surface our \\$PREFIX/etc/resolv.conf\" \\\n    \
             \"#   there.\" \\\n    \
             \"exec env -u LD_PRELOAD $PREFIX/bin/proot -b \\\"$PREFIX/etc/resolv.conf:/etc/resolv.conf\\\" $CLAUDE_BIN \\\"\\$@\\\"\" \\\n             > \"$PREFIX/bin/claude\"\n\
         chmod +x \"$PREFIX/bin/claude\"\n\
         \n\
         echo\n\
         echo \"done. now run:  claude --version\"\n",
    );
